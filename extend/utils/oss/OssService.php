...
class OssService
{
	 ...
	 */
	public static function OssUpload($file,$rename,$shop_id_folder = ''){
		if($rename == 2){
			$filepath = app('http')->getName().'/'.$shop_id_folder.date(config('my.upload_subdir')).'/'.$file->getOriginalName(); //上传路径
		}else{
			$filepath = app('http')->getName().'/'.$shop_id_folder.date(config('my.upload_subdir')).'/'.doOrderSn('000').'.'.$file->extension();
		}
