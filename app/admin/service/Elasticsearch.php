...
class Elasticsearch
{
    ...
    public function search($params, $options = []) {
        ...
        try {
            ...
            if ($options['last_id'] > 0) {
                $searchAfter = $this->getSearchAfterValues(
                    $options['last_id'],
                    $options['order_field'],
                    $options['order_direction'],
                    $options['primary_key']
                ...
                if (!empty($searchAfter)) {
                    $esParams['body']['search_after'] = $searchAfter;
                }
            }
            // dump($esParams);die;
            // 7. 执行查询
        ...
        if (!empty($whereConditions)) {
            foreach ($whereConditions as $condition) {
                if (isset($condition[0], $condition[1], $condition[2])) {
                    $filter[] = ['term' => [$condition[0] => $condition[2]]];
                }
            }
        }
    ...
    private function buildSort($orderField, $orderDir, $primaryKey) {
        ...
        // 主排序字段
        if ($orderField && $orderField !== $primaryKey) {
            $sort[] = [$orderField => ['order' => $orderDir]];
        }

        // 确保包含主键作为第二排序字段，保证排序唯一性
    ...
    private function getSearchAfterValues($lastId, $orderField, $orderDir, $primaryKey) {
        try {
            ...
            if ($orderField && $orderField !== $primaryKey && isset($lastDoc['_source'][$orderField])) {
                $fieldValue = $lastDoc['_source'][$orderField];
    ...
    private function buildResponse($list, $total, $options) {
        ...
        return [
            ...
            'per_page' => $options['limit'],
            'current_page' => $options['page'],
            'last_page' => $total > 0 ? ceil($total / $options['limit']) : 1,
