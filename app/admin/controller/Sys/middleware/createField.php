...
class createField extends Admin
{
    ...
    public function handle($request, \Closure $next)
    {
        ...
        if($dbType === 'mysql' && (int)($data['create_table_field'] ?? 0) === 1){
            if($menuInfo['page_type'] == 1){
                ...
                $indexData = $hasIndexInput ? $requestedIndexData : '';
                ...
                $syncFieldName = $fieldName;
            }
            if($menuInfo['page_type'] == 2 && (int)($data['type'] ?? 0) == 30){
                $fieldName = $data['field'];
                $columnInfo = self::getTableColumn($tableName, $fieldName, $connect);
                $hasIndexInput = array_key_exists('indexdata', $data);
                $requestedIndexData = $hasIndexInput ? self::normalizeIndexValue($data['indexdata']) : '';
        ...
        if (!empty($syncFieldName)) {
            self::syncFieldDefaultValueByMenuField((int)$syncMenuId, $syncFieldName, $tableName, $connect);
        }
    ...
    private static function syncFieldDefaultValueByMenuField(int $menuId, string $fieldName, string $tableName, string $connect): void
    {
        $columnInfo = self::getTableColumn($tableName, $fieldName, $connect);
        if (empty($columnInfo)) {
            return;
        ...
        $currentValidate = Field::where('menu_id', $menuId)->where('field', $fieldName)->value('validate');
        $validate = self::syncValidateByNotNull($currentValidate, $isNotNull);
        Field::where('menu_id', $menuId)->where('field', $fieldName)->update([
